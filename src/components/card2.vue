<template>
    <div class="card" v-if="playloads.asset==='BTCUSDT'">
        <div class="card-body">
            <div class="d-flex flex-row justify-content-center">
                <p class="timestamp text-center">
                    {{ dateFormate(current.from_timestamp) }}
                </p>
                <span>-</span>
                <p class="timestamp text-center">
                    {{ dateFormate(current.to_timestamp) }}
                </p>
            </div>

            <div class="line"></div>
            <div class="trends text-center">
                <img class="img-fluid" :src="trendStat(current.trend)" alt="Ranging" />
            </div>
            ...
            <div class="action result">
                <img class="img-fluid p-0 me-1" :src="resultState(current.result)" alt="" />
                <b class="fs-16 fw-700">{{ current.result }}</b>
            </div>
        </div>
    </div>
